...
with st.sidebar:
    ...
    for x in ["Extraction + semantic chunking","Gemini embeddings + Pinecone RAG","Query-driven analysis","Optional web research","Editable PDF / PPT / DOCX / TXT","Conversational editing + versioning"]: st.caption(f"• {x}")
...
if not st.session_state.current_artifact:
    ...
    with files:
        ...
        if paths:
            ...
            for p in paths: st.markdown(f'<div class="file-item">📄 {Path(p).name}</div>',unsafe_allow_html=True)
        else: st.markdown('<div class="empty-files">No file uploaded yet</div>',unsafe_allow_html=True)
    ...
    if st.button("✦  Generate Document  →",type="primary",use_container_width=True):
        paths=getattr(st.session_state,"upload_paths",[]); mapped="Auto" if output_type=="Auto" else {"PDF":"pdf","PPT":"pptx","DOCX":"docx","TXT":"txt"}[output_type]
        if not paths: st.error("Please upload a source document first.")
            ...
            with st.spinner("Ingesting → embedding → retrieving → analyzing → generating → validating…"): result=st.session_state.query_pipeline.run(query,query,paths,mapped)
...
last=max((i for i,m in enumerate(st.session_state.messages) if m.get("role")=="assistant"),default=-1)
for i,m in enumerate(st.session_state.messages):
    with st.chat_message(m["role"]):
        st.markdown(m["content"])
        if i==last and m.get("artifact"):
...
if st.session_state.current_artifact:
    ...
    with c1: undo=st.button("↩ Undo",use_container_width=True)
    with c2: restore=st.button("⭯ Original",use_container_width=True)
    ...
    if prompt:
        ...
        with st.chat_message("assistant"):
            ...
            if result.get("artifact"):
                st.session_state.current_artifact=result["artifact"]; a=result["artifact"]

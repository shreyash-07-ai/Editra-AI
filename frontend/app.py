...
st.markdown("""<style>.block-container{padding-top:1.2rem;max-width:1200px}.chat-title{font-size:2rem;font-weight:700}.chat-subtitle{color:#777}</style>""",unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if st.button("＋ New chat",use_container_width=True):
        st.session_state.messages=[]; st.session_state.current_artifact=None; st.session_state.pop("upload_paths",None); st.rerun()
    st.divider(); st.markdown("### 1. Upload source")
    uploads=st.file_uploader("DOCX, PDF, PPTX, PPT, XLSX, CSV, PNG, JPG, JPEG",type=["docx","pdf","pptx","ppt","xlsx","csv","png","jpg","jpeg"],accept_multiple_files=True)
    if uploads:
        st.session_state.upload_paths=[str(st.session_state.store.save_upload(up.name,up.getvalue())) for up in uploads]
        st.success(f"{len(uploads)} file(s) ready")
    if st.session_state.current_artifact:
        st.divider(); st.markdown(f"### Current version: v{st.session_state.current_artifact['version']}")
        c1,c2=st.columns(2)
        with c1: undo_clicked=st.button("↩ Undo last change",use_container_width=True)
        with c2: restore_clicked=st.button("⭯ Restore original",use_container_width=True)
    else: undo_clicked=restore_clicked=False
    st.divider(); st.markdown("### Capabilities")
    for x in ["• Extraction + semantic chunking","• Gemini embeddings + Pinecone RAG","• Query-driven semantic analysis","• Optional web research","• NEW editable DOCX / PPTX / XLSX","• Conversational editing + versioning"]: st.caption(x)

st.markdown('<div class="chat-title">Editra AI</div>',unsafe_allow_html=True)
st.markdown('<div class="chat-subtitle">Upload source material, describe it, enter a query, and generate a NEW editable artifact from retrieved evidence.</div>',unsafe_allow_html=True)

if not st.session_state.current_artifact:
    st.markdown("### Query-driven generation")
    description=st.text_area("2. Document description",placeholder="Describe the document type, purpose, important sections, metrics and terminology.",height=90)
    query=st.text_area("3. What should the NEW document contain?",placeholder="Example: Analyze the key numerical findings and explain their significance.",height=100)
    output_type=st.selectbox("4. Output type",["Auto","Report / DOCX","Summary / DOCX","Analysis / DOCX","Presentation / PPTX","Spreadsheet / XLSX"])
    if st.button("Generate NEW document",type="primary",use_container_width=True):
        paths=getattr(st.session_state,"upload_paths",[])
        mapped="Auto" if output_type=="Auto" else ("pptx" if "PPTX" in output_type else "xlsx" if "XLSX" in output_type else "docx")
        if not paths: st.error("Upload a source document first.")
        elif not query.strip(): st.error("Enter a query first. The query controls the new output.")
        else:
            with st.spinner("Ingesting → embedding → retrieving → analyzing → generating → validating…"):
                result=st.session_state.query_pipeline.run(description,query,paths,mapped)
...
for i,m in enumerate(st.session_state.messages):
    with st.chat_message(m["role"]):
        ...
        if i==last_assistant_index and m.get("artifact"):
            a=m["artifact"]; st.markdown(f"**Output:** {a['filename']} — version {a['version']}")
...
if st.session_state.current_artifact:
    ...
    if undo_clicked: prompt="undo the last change"
    elif restore_clicked: prompt="go back to the original"
    if prompt:
        ...
        with st.chat_message("assistant"):
            ...
            if result.get("artifact"):
                st.session_state.current_artifact=result["artifact"]
                a=result["artifact"]; st.markdown(f"**Output:** {a['filename']} — version {a['version']}")

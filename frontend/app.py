...
if not st.session_state.current_artifact:
    ...
    if paths:
        ...
        for path in paths:
            st.markdown(
                f'<div class="file-item">📄 <strong>{Path(path).name}</strong><span>Ready</span></div>',
...
if st.session_state.current_artifact:
    ...
    if latest:
        ...
        st.markdown(
            f'<div class="result-message">{latest.get("content", "Document generated successfully.")}</div>',
            unsafe_allow_html=True,
        )
        st.download_button(
